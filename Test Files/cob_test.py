...
def align_with(mob1, mob2, vertical=True):
    '''
...
class test(Scene):
    def construct(self):
        self.play(FadeIn(create_plane()))

class TestTransfLabels(LinearTransformationScene):
    CONFIG = {
...
class TransformOneTwo(LinearTransformationScene):
    ...
    def construct(self):
        ...
        inv_matrix = Matrix(self.inverse_matrix)
...
class IntroduceSimilarity(Scene):
    ...
    def construct(self):
        similarity_text = TexMobject("\\Large\\textbf{Similarity}")
        sim_formula = [
            "\\Large \\text{A}", 
            " = ", 
            "\\text{P}", 
        ...
        sim_formula = TexMobject(*sim_formula)
        sim_formula.set_color_by_tex_to_color_map({
            "A": PURPLE,
            "P": BLUE,
            "B": RED
        })
        sim_formula.shift(UP)
        ...
        arrows = [
            Arrow(explanations[i - 1].get_top(), sim_formula[i - 1].get_bottom(), color = ORANGE)
            for i in range(len(sim_formula), 0, -1) ## Reversing the order of the arrows
        ]
        
        del explanations[-2], arrows[-2] ## We do not need the equal sign nor the text

        self.play(Write(similarity_text))
        self.play(similarity_text.to_edge, UP)
        self.play(Write(sim_formula))
        self.wait()
